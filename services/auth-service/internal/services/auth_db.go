...
func (s *AuthService) RegisterUser(user *models.User) error {
	...
	if err != nil {
		...
		return shared_errors.DatabaseError("EMAIL_CHECK_FAILED", "Failed to check email existence")
	}

	if exists {
		return shared_errors.ConflictError("EMAIL_EXISTS", "Email already registered")
	}
	...
	if err != nil {
		...
		return shared_errors.InternalError("PASSWORD_HASH_FAILED", "Failed to hash password")
	}
	...
	if err != nil {
		...
		return shared_errors.DatabaseError("USER_CREATION_FAILED", "Failed to create user")
	}
...
func (s *AuthService) LoginUser(email, password string) (string, error) {
	...
	if err != nil {
		s.logger.Error("Failed to generate JWT",
			zap.String("user_id", user.ID),
			zap.Error(err))
		return "", shared_errors.InternalError("TOKEN_GENERATION_FAILED", "Failed to generate authentication token")
	}
...
func (s *AuthService) GenerateRefreshToken(userID string) (string, error) {
	...
	// Store in database
	refreshToken := models.NewRefreshToken(userID, token, time.Now().Add(time.Hour*24*7)) // 7 days
	ctx := context.Background()
...
func (s *AuthService) RefreshAccessToken(refreshTokenString string) (string, error) {
	...
	if err != nil {
		return "", shared_errors.AuthenticationError("INVALID_REFRESH_TOKEN", "Invalid or expired refresh token")
	}

	// Get user
	user, err := s.userRepo.GetByID(ctx, refreshToken.UserID)
	if err != nil {
		return "", shared_errors.AuthenticationError("USER_NOT_FOUND", "User not found")
	}
...
func (s *AuthService) UpdateUserProfile(ctx context.Context, userID string, req *models.UpdateProfileRequest) error {
	...
	if err != nil {
		s.logger.Error("Failed to update user profile",
			...
			zap.Error(err))
		return shared_errors.DatabaseError("PROFILE_UPDATE_FAILED", "Failed to update profile")
	}
...
func (s *AuthService) LogoutAll(userID string) error {
	...
	if err != nil {
		s.logger.Error("Failed to revoke all refresh tokens",
			zap.String("user_id", userID),
			zap.Error(err))
		return shared_errors.DatabaseError("LOGOUT_ALL_FAILED", "Failed to logout from all devices")
	}
