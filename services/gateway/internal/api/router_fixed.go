...
func SetupRouter(logger *zap.Logger) *gin.Engine {
	...
	// SECURE CORS middleware - Production ready
	router.Use(func(c *gin.Context) {
		...
		allowedOrigins := []string{
			...
			"https://admin.blytz.app",
		}
		
		// Add development origins if in dev mode
		if os.Getenv("NODE_ENV") == "development" || os.Getenv("ENVIRONMENT") == "development" {
		...
		if origin != "" {
			...
			// No origin header (likely from curl/API clients)
			allowed = true
		}
		...
		c.Header("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Correlation-ID, X-Client-Version, X-Request-ID")
